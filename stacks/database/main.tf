...
data "terraform_remote_state" "networking" {
  ...
  config = {
    ...
    key    = format("%s/%s/%s", var.team_name, var.environment, "networking")
  }
}
...
data "terraform_remote_state" "application" {
  ...
  config = {
    region = "ap-southeast-1"
    bucket = "infra-training-state-2022"
    key    = format("%s/%s/%s", var.team_name, var.environment, "application")
  }
}

data "aws_eks_cluster" "default" {
  name =  data.terraform_remote_state.application.outputs.eks_cluster_id
}
...
module "cluster" {
  ...
  allowed_security_groups = [data.terraform_remote_state.application.outputs.cluster_primary_security_group_id]
